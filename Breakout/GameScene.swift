...
//  Breakout
//
//  Created by Justin Rebbeck on 3/18/24.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createBackground() {
        ...
        for i in 0...1 {
            ...
            let moveReset = SKAction.moveBy(x: 0, y: starsBackground.size.height, duration: 20)
            let moveLoop = SKAction.sequence([moveDown, moveReset])
            let moveForever = SKAction.repeatForever(moveLoop)
            starsBackground.run(moveForever)
        }
    }
    func makeBall() { // remove the ball (if it exists)
        ...
        // physcis shape matches the ball
        ball.physicsBody = SKPhysicsBody(circleOfRadius: 10)
        // ignores all forces and impulses
        ball.physicsBody?.isDynamic = false
        // use percise collision protection
        ball.physicsBody?.usesPreciseCollisionDetection = true
        // no loss of energy from friction
        ball.physicsBody?.friction = 0
        // gravity is no longer a factor
    ...
    func makeLoseZone() {
        loseZone = SKSpriteNode(color: .clear, size: CGSize(width: frame.width, height: 50))
        loseZone.position = CGPoint(x: frame.midX, y: frame.midY - 425)
